...
module.exports.execute = async (client, message, args) => {
  ...
  message.channel.send(`${resital} ${time} boyunca metin kanallarında susturuldu. \`#${resital1}\``)
     kdb.push(`kullanici.${resital.id}.sicil`, {
      Yetkili: message.author.id,
      Tip: "MUTE",
      Sebep: reason,
      Zaman: Date.now()});
  resital.roles.add(`${ayar.muteRolü}`)
  client.channels.cache.get(ayar.mutelog).send(embed.setDescription(`${resital} \`${resital.user.id}\` üyesi metin kanallarında susturuldu.
  ...
  setTimeout(async () => {
    resital.roles.remove(`${ayar.muteRolü}`)
    client.channels.cache.get(ayar.mutelog).send(embed.setDescription(`${resital} \`${resital.user.id}\` üyesinin metin kanallarında bulunan ${time} susturulması otomatik kaldırıldı.
...
module.exports.configuration = {
  name: "mute",
  aliases: ['mute'],
  usage: "mute @Resitâl/ID [süre] [sebep]",
