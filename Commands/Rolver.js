...
module.exports.execute = async (client, message, args) => {
  ...
  let olumlu = new MessageEmbed().setTitle(message.member.displayName, message.author.avatarURL({dynamic: true})).setTimestamp().setColor("GREEN").setFooter('Resital 🤍 FlowArt');
  ...
  let resart = args[0];
  if (!resart || !member) return message.channel.send(olumsuz.setDescription(`Komutu doğru kullanmalısın! \`Örnek: ${ayar.prefix || ""}rol vip @üye\``).setFooter(`${ayar.prefix || ""}rol bilgi | kullanarak bilgi alabilirsiniz.`)).then(x => x.delete({timeout: 12000}));
...
module.exports.configuration = {
    name: "rol",
    aliases: ["rol", "r"],
    usage: "rol vip/muzisyen/vokal/terapist/soruncözücü/lovers @Resitâl/ID",
